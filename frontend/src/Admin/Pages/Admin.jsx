    ...
    const activityColumns = [
        {
            ...
            render: (item) => (
                <div className="flex flex-col">
                    <span className={`font-bold ${isDark ? 'text-slate-100' : 'text-slate-900'}`}>{item.name}</span>
        ...
        },
        {
            ...
            accessor: 'status',
            render: (item) => (
                <span className={`px-2 py-1 rounded-lg text-[10px] font-bold ${item.status === 'Sold' ? 'bg-emerald-500/10 text-emerald-500' :
        ...
        },
        {
            header: 'Amount',
            accessor: 'amount',
            render: (item) => <span className={`font-bold ${isDark ? 'text-slate-100' : 'text-slate-900'}`}>{item.amount}</span>
        },
        {
            header: 'Time',
            accessor: 'time',
            render: (item) => <span className="text-xs text-slate-500 italic">{item.time}</span>
        }
    ];
